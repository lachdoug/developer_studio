    ...
    class Components
      class Sources
        ...
        def set_collection_for(collection_params)
          @collection = collection_params.map { |i, item_params| Source.new( blueprint_section, item_params.merge( {index: i} ) ) }
        end

        def build
          Source.new( blueprint_section, {new_record: true, index: @collection.size} ).tap do |item|
            @collection << item.tap(&:build_install_script)
          end
        end
